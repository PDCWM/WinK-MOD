...
public final class BlockUtils
{
	...
	public static LinkedList<BlockPos> findBlocksNearEntity(int blockId, int blockMeta, int distance) {
	...
	public static LinkedList<XRayBlock> findBlocksReNearEntity(int distance) {
			...
			for (int z = (int) Wrapper.INSTANCE.player().posZ - distance; z <= (int) Wrapper.INSTANCE.player().posZ + distance; ++z) {
				...
				block: for (int y = 0; y <= height; ++y) {
					...
					for(XRayData data : XRayManager.xrayList) {
						if (id == data.getId() && meta == data.getMeta()) {
	...
	public static LinkedList<XRayBlock> findBlocksAINearEntity(int distance) {
		LinkedList<XRayBlock> blocks = new LinkedList<XRayBlock>();
		...
		for (int x = (int) Wrapper.INSTANCE.player().posX - distance; x <= (int) Wrapper.INSTANCE.player().posX + distance; ++x) {
			for (int z = (int) Wrapper.INSTANCE.player().posZ - distance; z <= (int) Wrapper.INSTANCE.player().posZ + distance; ++z) {
				...
				int AiCount = 0;
				LinkedList<XRayBlock> TBlocks = new LinkedList<XRayBlock>();
				block: for (int y = 0; y <= height; ++y) {
					if (AiCount > 7){
						TBlocks.clear();
						break block;
