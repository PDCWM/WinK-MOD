...
//			entitySize = new EntitySize(0.4F, 0.5F);
	...
	/*private static boolean weiKeModsIsProning(EntityPlayer ep) {
		try {
			...
			Method method = g.getMethod("isProning", EntityPlayer.class);
			return !(boolean)method.invoke(g, ep);
		} catch (Exception e) {
	...
	public static void assistFaceEntity(Entity entity, float yaw, float pitch, boolean type) {
		...
		if (entity instanceof EntityLivingBase) {
			/*ChatUtils.warning(String.format("EntityPlayer %f %f",
			...
			double height;
			if(type) {
				height = entityLivingBase.height * 0.85F;
			} else {
				height = entityLivingBase.height * 0.10F;
			}
			yDifference = entityLivingBase.posY + height
					- (Wrapper.INSTANCE.player().posY + Wrapper.INSTANCE.player().getEyeHeight());
		} else {
			//ChatUtils.warning(entity.toString());
